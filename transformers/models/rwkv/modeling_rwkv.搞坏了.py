...
def load_wkv_cuda_kernel(context_length):
    ...
    print(f"Loading CUDA kernel for RWKV at context length of {context_length}.")
...
class RwkvLinearAttention(torch.autograd.Function):
    ...
    def forward(ctx, time_decay, time_first, key, value, state=None, return_state=False):
        ...
        if return_state or state is not None:
            ...
            forward_func(time_decay, time_first, key, value, output, state.clone())
        else:
            # print('这里3')
...
def rwkv_linear_attention_cpu(time_decay, time_first, key, value, state=None, return_state=False):
    ...
    if state is None:
        # 没有CPU + state = None的情况, 生成一个state
        num_state = torch.zeros_like(key[:, 0], dtype=torch.float32)
        den_state = torch.zeros_like(key[:, 0], dtype=torch.float32)
        max_state = torch.zeros_like(key[:, 0], dtype=torch.float32) - 1e38
    else:
        # print('I am here')
        num_state, den_state, max_state = state # 从state取
    # For numerical stability
    #    real_numerator_state = num_state * torch.exp(max_state)
    #    real_denominator_state = den_state * torch.exp(max_state)
    time_decay = -torch.exp(time_decay) # NOTE: detach


    # 应该没有问题的，因为如果不带state是非常正常的输出
    for current_index in range(seq_length):
        ...
        ee1 = torch.exp(max_state + time_decay - max_for_state)
        ee2 = torch.exp(current_key - max_for_state)
        num_state_new = ee1 * num_state + ee2 * current_value
        den_state_new = ee2 * den_state + ee2
        max_state_new = max_for_state

    # NOTE: 那唯一的问题根源只能在这里
    if return_state or state is not None:
        state = [num_state_new.clone(), den_state_new.clone(), max_state_new.clone()]
...
def rwkv_linear_attention(time_decay, time_first, key, value, state=None, return_state=False):
    ...
    if rwkv_cuda_kernel is None:
        # print('怎么会没有kernel?')
        pass
    else:
        for t in [time_decay, time_first, key, value]:
            # print(t.device.type)
            pass
        # print('确认结束')
    if state:
    ...
    if rwkv_cuda_kernel is None or no_cuda or one_token:
        # print('NONONONONO')
        # NOTE: 没有GPU的情况下进入这里
        # print('rwkv_linear_attention_cpu')
...
class RwkvSelfAttention(nn.Module):
    def __init__(self, config, layer_id=0):
        ...
        #print(f'Ninja: {is_ninja_available()}')
        # print(f'torch cuda: {is_torch_cuda_available()}')
    ...
    def forward(self, hidden, state=None, use_cache=False):
        ...
        if state is not None:
            layer_state = tuple(s[:, :, self.layer_id].clone() for s in state[2:])
        else:
            layer_state = None
...
class RwkvModel(RwkvPreTrainedModel):
    ...
    def forward(
        ...
        hidden_states = inputs_embeds # hidden state是由input_ids生成的
